...
# ================= CONFIGURAÇÕES =================
TEMPO_OCIOSO_ALVO = 6      # 1 minuto sem mexer para ativar a pegadinha
INTERVALO_MOVER_MOUSE = 4  # Intervalo para mover o mouse (Anti-Teams)
...
def log(mensagem, categoria="INFO"):
    """Exibe logs formatados com horário no terminal"""
...
def ao_pressionar_tecla(key):
    ...
    if janelas_hacker:
        if key == keyboard.Key.esc:
            log("Tecla ESC pressionada pelo usuário! Interrompendo Modo Hacker...", "CANCELAR")
...
def fechar_todas_as_telas():
    ...
    log("Todas as janelas do Modo Hacker foram encerradas.", "SISTEMA")
...
def verificar_sistema():
    ...
    # Verifica se atingiu o tempo de ociosidade sem o modo hacker ativo
    if tempo_inativo >= TEMPO_OCIOSO_ALVO and not janelas_hacker:
        log(f"Ausência humana detectada! Inativo por {int(tempo_inativo)}s (Limite: {TEMPO_OCIOSO_ALVO}s)", "ALERTA")
        log("Iniciando Modo Hacker / Tela de Ransomware...", "HACKER")
        criar_janelas_hacker()

    # Movimentação preventiva do mouse (Anti-Teams)
    if tempo_atual - ultimo_movimento_mouse >= INTERVALO_MOVER_MOUSE:
        ...
        log(f"Mouse movimentado preventivamente para ({novo_x}, {novo_y}) [Anti-Teams]", "MONITOR")
...
def criar_janelas_hacker():
    ...
    log(f"Mapeando telas: {len(monitores)} monitor(es) detectado(s).", "HACKER")
...
def animar_texto():
    ...
    if texto_atual_idx < len(COMANDOS_HACKER):
        ...
        log(f"Exibindo comando: {cmd}", "TERMINAL")
...
def animar_barra():
    ...
    if progresso_atual < limite_maximo:
        ...
        log("Progresso atingiu 100%! Ativando alerta piscante final.", "HACKER")
        piscar_tela_final()
    else:
        loop_barra_id = root.after(300, animar_barra)
...
def disparar_popups_falsos():
    ...
    try:
        ...
        log(f"Pop-up gerado na zona '{zona}' (Total na tela: {len(janelas_popups)})", "POPUP")
        ...
        while len(janelas_popups) > MAX_POPUPS:
            ...
            try:
                ...
                pass
            log(f"Limite máximo de {MAX_POPUPS} pop-ups atingido. Fechando o mais antigo.", "POPUP")
    except:
        pass
...
def sair_programa(icon, item):
    log("Encerrando aplicação via Ícone na Bandeja...", "SAIR")
    listener_mouse.stop()
...
if __name__ == "__main__":
    print("==========================================================")
    print("      ANTI-AUSENTE HACKER - MODO CONSOLE / LOGS           ")
    print("==========================================================")
    log(f"Iniciando monitoramento de ociosidade...", "INÍCIO")
    log(f"Host identificado: {NOME_PC} | Usuário: {NOME_USUARIO}", "SISTEMA")
    log(f"Tempo de Ociosidade para Ativar Modo Hacker: {TEMPO_OCIOSO_ALVO}s", "CONFIG")
    log(f"Intervalo de Movimento Preventivo de Mouse: {INTERVALO_MOVER_MOUSE}s", "CONFIG")
    print("----------------------------------------------------------")
    ...
    try:
        ...
        log("Interrupção via Teclado (Ctrl+C). Encerrando...", "SAIR")
        listener_mouse.stop()
        listener_teclado.stop()
        if tray_icon:
            tray_icon.stop()